<?php
// pages/dashboard-libraire.php
require_once __DIR__ . '/../config/db.php';
require_once __DIR__ . '/../config/auth.php';
authorize(['libraire', 'admin']);

try {
    $stmt_ca    = $pdo->query("SELECT SUM(total_montant) AS ca FROM ventes WHERE statut='Payé'");
    $ca_mensuel = $stmt_ca->fetch()['ca'] ?? 0;

    $stmt_today  = $pdo->query("SELECT COUNT(*) AS total_today FROM ventes WHERE DATE(date_vente)=CURDATE() AND statut='Payé'");
    $ventes_today = $stmt_today->fetch()['total_today'] ?? 0;

    $stmt_stock = $pdo->query("SELECT COUNT(*) AS total_low FROM livres WHERE quantite_stock <= seuil_alerte");
    $stock_low  = $stmt_stock->fetch()['total_low'] ?? 0;

    // Emprunts actifs du jour
    $stmt_emp = $pdo->query("SELECT COUNT(*) AS total FROM emprunts WHERE statut='En cours'");
    $emprunts_actifs = $stmt_emp->fetch()['total'] ?? 0;
    ...
    $ca_mensuel = $ventes_today = $stock_low = $emprunts_actifs = 0;
    $ventes_recentes = [];
    $last_7_days = []; $chart_values = [];
}

$page_title   = 'Espace Libraire';
...
?>
    <div class="container-fluid py-5 px-lg-5">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-5 gap-3">
            <div>
                <h1 class="h2 font-weight-bold mb-1" style="color:var(--text-heading)">Espace Libraire</h1>
                <p class="text-muted small mb-0 mt-1">Bonjour, <strong class="text-primary"><?= htmlspecialchars($_SESSION['user_name']) ?></strong> 👋. Voici l'état actuel de votre librairie.</p>
            </div>
            <a href="prets.php" class="btn btn-primary rounded-pill px-4 py-2 shadow-sm fw-bold">
                <i class="fas fa-shopping-cart me-2"></i>Encaisser une vente
            </a>
        </div>
        ...
        <div class="row g-4 mb-5">
            <div class="col-xl-3 col-md-6">
                <div class="card p-4 h-100 shadow-sm border-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fc);">
                    ...
                    <p class="small mb-0 fw-bold" style="color:var(--primary)">Chiffre d'Affaires</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="card p-4 h-100 shadow-sm border-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fc);">
                    ...
                    <h3 style="font-weight:800;color:var(--text-heading)"><?= $ventes_today ?></h3>
                    <p class="small mb-0 fw-bold" style="color:var(--accent)">Ventes du jour</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="card p-4 h-100 shadow-sm border-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fc);">
                    <div class="stat-card-icon mb-3 shadow-sm d-flex align-items-center justify-content-center" style="background:rgba(99,102,241,.1);color:#6366f1;width:50px;height:50px;border-radius:12px;"><i class="fas fa-book-reader fs-5"></i></div>
                    <h3 style="font-weight:800;color:var(--text-heading)"><?= $emprunts_actifs ?></h3>
                    <p class="small mb-0 fw-bold" style="color:#6366f1">Emprunts Actifs</p>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="card p-4 h-100 shadow-sm border-0" style="background: linear-gradient(145deg, #ffffff, #fdf4e3);">
                    <div class="stat-card-icon mb-3 shadow-sm d-flex align-items-center justify-content-center" style="background:rgba(245,158,11,.1);color:var(--warning);width:50px;height:50px;border-radius:12px;"><i class="fas fa-box fs-5"></i></div>
                    <h3 style="font-weight:800;color:var(--text-heading)"><?= $stock_low ?></h3>
                    <p class="small mb-0 fw-bold" style="color:var(--warning)">Stock Faible <span class="text-danger fw-normal">(À vérifier)</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="row g-4 mb-5">
            <!-- Actions Rapides -->
            <div class="col-lg-4">
                <div class="card p-4 h-100 shadow-lg border-0 text-white" style="background: linear-gradient(135deg, #10b981, #059669) !important; border-radius: 16px;">
                    <h5 class="mb-4 text-white" style="font-weight:700;">Opérations Courantes</h5>
                    <div class="d-grid gap-3">
                        <a href="prets.php" class="btn btn-light btn-lg text-success shadow-sm rounded-pill text-start px-4" style="transition: transform 0.2s;">
                        ...
                        <a href="livres.php" class="btn btn-outline-light btn-lg rounded-pill text-start px-4" style="transition: transform 0.2s;">
                            <i class="fas fa-plus me-3"></i> Entrée de Stock
                        </a>
                        <a href="../index.php" class="btn btn-outline-light btn-lg rounded-pill text-start px-4" style="transition: transform 0.2s;">
                            <i class="fas fa-store me-3"></i> Accès Boutique
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-8">
                <div class="card p-4 h-100 shadow-sm border-0">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h5 class="mb-0" style="font-weight:700;color:var(--text-heading)">Activité de la semaine</h5>
        ...
        <div class="card p-0 shadow-sm border-0 overflow-hidden mb-5" style="border-radius: 16px;">
            <div class="card-header bg-white p-4 border-bottom-0">
                <h5 class="mb-0" style="font-weight:700;color:var(--text-heading)">Vos dernières transactions</h5>
            </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody style="color:var(--text-body)">
                        <?php if (count($ventes_recentes) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="rounded-circle bg-success text-white d-flex align-items-center justify-content-center me-3 shadow-sm" style="width: 32px; height: 32px; font-size: 0.8rem; font-weight:bold;">
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="5" class="text-center py-5 text-muted">Aucune vente enregistrée récemment.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            <div class="card-footer bg-white p-3 text-center border-top-0">
                <a href="prets.php" class="btn btn-link text-decoration-none text-success fw-bold">Journal complet des ventes <i class="fas fa-arrow-right ms-1"></i></a>
            </div>
        </div>
    </div>
<?php 
